...
# 1. A Dashboard for stocks.
# 2. Refresh the data.
# 3. Import user settings/information.
# 4. Display and Publish results.
import time
import ib_insync as ib_
import numpy as np
import pandas as pd
...
# Fetching historical data when market is closed for testing purpose
# and EMA values:
# For 55 EMA and 4 EMA we need previous 55 and 4 candles of 3 min each,
# so they cant be obtained during only live real time market data,
# we need historical data as below:
...
while TimeNow <= EndTime:
    ...
    Long Entry: the (1mim/3min) Bar candle has closed ABOVE the 55EMA
    ...
    if order_status[0] == 0 and order_status[1] == 0:
        if last_close > ema_value_4 and ema_value_4 > ema_value_55:
            ...
            options_chains = ib.reqSecDefOptParams(
                stock.symbol, "", stock.secType, stock.conId
            )
            print("options_chains: ", options_chains)
            # In this case we're only interested in the weekly options trading on SMART:
            option_weekly_chains = next(
                c
                for c in options_chains
                if c.tradingClass == "TSLA" and c.exchange == "SMART"
            )
            ...
            print(
                "Number of contracts for eligible strikes in CALL:",
            ...
            if len(option_contracts):
                ...
                dps = (
                    str(ib.reqContractDetails(option_contract)[0].minTick + 1)[
                        ::-1
                    ].find(".")
                    - 1
                )
                lmtPrice = round(
                    CurrentValue
                    - ib.reqContractDetails(option_contract)[0].minTick * 2,
                    dps,
                )
                print("Trading Long")
                # building order
                entry_order = ib.bracketOrder(
                    ...
                    qty,
                    limitPrice=lmtPrice,
                    takeProfitPrice=CurrentValue
                    + CurrentValue * (profit_booking_percent / 100),
                    stopLossPrice=CurrentValue
                    - CurrentValue * (stop_loss_percent / 100),
                )
    ...
    i.e. Short Entry: the 3min Bar candle has closed below the 55EMA
    ...
    # Wait for 3 mins for next candle to form:
    print(
        "No condition met for this 3 min candle so waiting 3 mins for fetching the next candle."
    )
    ib.sleep(180)
    TimeNow = pd.to_datetime(ib.reqCurrentTime()).tz_convert("America/New_York")
